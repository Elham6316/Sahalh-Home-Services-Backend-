...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // SQL للتحقق من وجود البريد الإلكتروني في قاعدة البيانات
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $row['cust_password'])) {
            ...
            // التوجيه إلى الصفحة الرئيسية أو لوحة التحكم أو الصفحة الشخصية
            header("Location: profile.php");  // يمكنك تعديلها إلى الصفحة التي تريدها
            exit();
        } else {
    ...
    <div class="tab-content" id="authTabsContent">
      ...
      <div class="tab-pane fade show active" id="login" role="tabpanel" aria-labelledby="login-tab">
        <form method="POST" action="login.php" >
          ...
          <button type="button" class="btn-primary" onclick="adminlogin();">تسجيل الدخول</button>
        </form>
      </div>
